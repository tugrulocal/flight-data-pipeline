...
[ "$#" -eq 2 ] || fail "Kullanım: $0 WAREHOUSE_KLASORU YENI_RAPOR.html"
...
[ ! -e "$report" ] || fail "Rapor zaten var; üzerine yazılmadı: $report"
mkdir -p "$project_root/spark/.ivy2" "$(dirname "$report")"
...
exec docker run --rm \
  ...
  /opt/flight-spark/jobs/generate_iceberg_report.py \
  --warehouse "/warehouse-parent/$warehouse_name" \
  --output "/report-parent/$report_name"
