...
[ "$#" -eq 2 ] || fail "Kullanım: $0 WAREHOUSE_KLASORU initial|append"
...
case "$phase" in initial|append) ;; *) fail "Aşama yalnız initial veya append olabilir." ;; esac
...
# Bu tek Docker container içindeki local[2] Spark'tır. Yalnız ayrı demo
# Iceberg tablosuna yazar; Bronze/Silver/Gold kaynak tabloları değiştirilmez.
exec docker run --rm \
  --cpus=2 \
  --memory=3g \
  -e HOME=/tmp \
  ...
  /opt/flight-spark/jobs/iceberg_snapshot_demo.py \
