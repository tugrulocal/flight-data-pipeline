...
# Bronze salt-okunur kaynak, Silver/Rejected hedefleri aynı warehouse içindedir.
# İş tekrarlandığında anti-join daha önce yazılan event_id'leri tekrar eklemez.
exec docker run --rm \
  --cpus=2 \
  --memory=3g \
  -e HOME=/tmp \
  ...
  /opt/flight-spark/jobs/iceberg_silver_refresh.py \
